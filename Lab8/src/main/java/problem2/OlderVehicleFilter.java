...
//    this.vehicles.stream().filter(vehicle -> vehicle.getYear() > FILTER_YEAR).forEach(vehicle ->
//        Vehicle.printMakeModelAndYear(vehicle));
//
//    //approach 2 -- store filtered vehicles into a list , and then print elements of a list
//    List<Vehicle> resultingVehicles = this.vehicles.stream().filter(vehicle -> vehicle.getYear() >
//        FILTER_YEAR).collect(Collectors.toList());
